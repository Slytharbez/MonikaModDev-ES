# TODO: Translation updated at 2026-03-31 15:02

# game/splash.rpy:191
translate spanish splashscreen_0320a168:

    # "[config.name] is a Doki Doki Literature Club fan mod that is not affiliated with Team Salvato."
    "[config.name] es un mod hecho por fans de Doki Doki Literature Club que no está afiliado a Team Salvato."

# game/splash.rpy:192
translate spanish splashscreen_12775544:

    # "It is designed to be played only after the official game has been completed, and contains spoilers for the official game."
    "Está diseñado para jugarse solo después de haber terminado el juego oficial, y contiene spoilers del mismo."

# game/splash.rpy:193
translate spanish splashscreen_e193832f:

    # "Game files for Doki Doki Literature Club are required to play this mod and can be downloaded for free at: http://ddlc.moe"
    "Se requieren los archivos de juego de Doki Doki Literature Club para jugar a este mod, los cuales pueden descargarse gratis en: http://ddlc.moe"

# game/splash.rpy:271
translate spanish after_load_78a4e9dc_1:

    # "The save file could not be loaded."
    "No se pudo cargar el archivo de guardado."

# game/splash.rpy:272
translate spanish after_load_5c5510ae_1:

    # "Are you trying to cheat?"
    "¿Estás intentando hacer trampa?"

# game/splash.rpy:272
translate spanish after_load_5c5510ae:

    # "Are you trying to cheat?"
    "¿Estás intentando hacer trampa?"

translate spanish strings:

    # game/splash.rpy:19
    old "This game is an unofficial fan work, unaffiliated with Team Salvato."
    new "Este juego es una obra de fans no oficial, sin afiliación con Team Salvato."

    # game/splash.rpy:21
    old "Please support Doki Doki Literature Club & Team Salvato."
    new "Por favor, apoya a Doki Doki Literature Club y a Team Salvato."

    # game/splash.rpy:22
    old "You are my sunshine,\nMy only sunshine"
    new "Eres mi sol,\nmi único sol"

    # game/splash.rpy:23
    old "I missed you."
    new "Te extrañé."

    # game/splash.rpy:24
    old "Play with me"
    new "Juega conmigo"

    # game/splash.rpy:25
    old "It's just a game, mostly."
    new "Es solo un juego, mayormente."

    # game/splash.rpy:26
    old "This game is not suitable for children\nor those who are easily disturbed?"
    new "¿Este juego no es apto para niños\no para personas que se perturben con facilidad?"

    # game/splash.rpy:27
    old "sdfasdklfgsdfgsgoinrfoenlvbd"
    new "sdfasdklfgsdfgsgoinrfoenlvbd"

    # game/splash.rpy:28
    old "null"
    new "null"

    # game/splash.rpy:29
    old "I have granted kids to hell"
    new "He condenado niños al infierno"

    # game/splash.rpy:30
    old "PM died for this."
    new "PM murió por esto."

    # game/splash.rpy:31
    old "It was only partially your fault."
    new "Solo fue tu culpa en parte."

    # game/splash.rpy:32
    old "This game is not suitable for children\nor those who are easily dismembered."
    new "Este juego no es apto para niños\no para personas que sean fácilmente descuartizadas."

    # game/splash.rpy:195
    old "By playing [config.name] you agree that you have completed Doki Doki Literature Club and accept any spoilers contained within."
    new "Al jugar a [config.name], aceptas que has completado Doki Doki Literature Club y aceptas cualquier spoiler contenido en él."

    # game/splash.rpy:196
    old "I agree."
    new "Acepto."
