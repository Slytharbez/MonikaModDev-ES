...
init 5 python:
    ...
    def mas_get_cat_label(cat):
        ...
        """
        if not cat or not isinstance(cat, basestring):
            return cat

        curr_lang = getattr(_preferences, "language", None) or renpy.translation.get_language()
        if curr_lang == "spanish":
            cat_key = cat.strip().lower()

            # 1. Search in MAS_CAT_TRANS dictionary (case and whitespace insensitive)
            if cat_key in MAS_CAT_TRANS:
                trans = MAS_CAT_TRANS[cat_key]
                try:
                    return renpy.substitute(trans)
                except Exception:
                    return trans

            # 2. Fallback: attempt translation via general Ren'Py string translation table
            tl_str = renpy.translation.translate_string(cat)
            if tl_str != cat:
                return tl_str

            tl_str_lower = renpy.translation.translate_string(cat_key)
            if tl_str_lower != cat_key:
                return tl_str_lower
