# TODO: Translation updated at 2026-03-31 15:02
 
# game/chess.rpy:716
translate spanish game_chess_f85b6700:

    # m "Let's continue our unfinished game."
    m 1eub "Continuemos nuestra partida pendiente."

# game/chess.rpy:782
translate spanish game_chess_3507e2a3:

    # m 2wfw "[player]!"
    m 2wfw "¡[player]!"

# game/chess.rpy:783
translate spanish game_chess_5e027179:

    # m 2wfx "You removed the save again."
    m 2wfx "Volviste a borrar la partida guardada."

# game/chess.rpy:785
translate spanish game_chess_8bc6286c:

    # m 2rfc "Let's just play chess at another time, then."
    m 2rfc "Mejor juguemos ajedrez en otro momento."

# game/chess.rpy:849
translate spanish game_chess_9be668f3:

    # m 1eua "We still have an unfinished game in progress."
    m 1eua "Todavía tenemos una partida pendiente."

# game/chess.rpy:854
translate spanish game_chess_load_check_b01706c4:

    # m 1efb "Get ready!"
    m 1efb "¡Prepárate!"

# game/chess.rpy:937
translate spanish mas_chess_remenu_0da28271:

    # m "How would you like to play?[('{fast}' if loopback else '')]" nointeract
    m "¿Cómo te gustaría jugar?[('{fast}' if loopback else '')]" nointeract

# game/chess.rpy:947
translate spanish mas_chess_remenu_6b6b37ac:

    # m 1ekc "...Alright, [player].{w=0.3} I was really looking forward to playing with you."
    m 1ekc "... De acuerdo, [player]. {w=0.3}Tenía muchas ganas de jugar contigo."

# game/chess.rpy:948
translate spanish mas_chess_remenu_51dbe566:

    # m 1eka "We'll play another time though, right?"
    m 1eka "Pero jugaremos en otro momento, ¿verdad?"

# game/chess.rpy:968
translate spanish mas_chess_remenu_e87a6bf1:

    # m 1eub "Sure! {w=0.2}{nw}"
    m 1eub "¡Claro! {w=0.2}{nw}"

# game/chess.rpy:969
translate spanish mas_chess_remenu_441eccc5:

    # extend 1eua "Naturally, {i}Normal Chess{/i} means standard chess."
    extend 1eua "Desde luego, el {i}Ajedrez Normal{/i} es el ajedrez estándar."

# game/chess.rpy:970
translate spanish mas_chess_remenu_2119bcd1:

    # m 3eua "Then there's {i}Randomized Chess{/i}, a mode based on {i}Really Bad Chess{/i}."
    m 3eua "Después tenemos el {i}Ajedrez Aleatorio{/i}, un modo basado en {i}Really Bad Chess{/i}."

# game/chess.rpy:971
translate spanish mas_chess_remenu_8bf8baa4:

    # m 3eub "We get completely random pieces which adds the factor of luck to make it fun for players of any skill level."
    m 3eub "Recibimos piezas completamente aleatorias, lo que añade el factor suerte para que sea divertido para jugadores de cualquier nivel."

# game/chess.rpy:972
translate spanish mas_chess_remenu_a701c0b9:

    # m 1eua "Alternatively, there's a more fair random chess mode called {i}Chess 960{/i}, also known as {i}Fischer Random Chess{/i}."
    m 1eua "También existe un modo aleatorio más justo llamado {i}Ajedrez 960{/i}, conocido además como {i}Ajedrez aleatorio de Fischer{/i}."

# game/chess.rpy:973
translate spanish mas_chess_remenu_4fe52604:

    # m 3eud "In this mode, the pieces in the back row are randomly shuffled while ensuring the bishops are placed on opposite color squares and the king is placed between two rooks."
    m 3eud "En este modo, las piezas de la fila inicial se distribuyen al azar, asegurando que los alfiles queden en casillas de distinto color y que el rey se ubique entre ambas torres."

# game/chess.rpy:974
translate spanish mas_chess_remenu_0a2b0e5d:

    # m 4hua "There's 960 possible starting positions, so it was called {i}Chess 960{/i}."
    m 4hua "Hay 960 posiciones iniciales posibles, por eso se llama {i}Ajedrez 960{/i}."

# game/chess.rpy:975
translate spanish mas_chess_remenu_dada6bbd:

    # m 1eua "{i}Chess 960{/i} allows players to avoid the complex opening theory while still testing their understanding of chess."
    m 1eua "El {i}Ajedrez 960{/i} permite a los jugadores evitar la compleja teoría de aperturas sin dejar de poner a prueba su comprensión del ajedrez."

# game/chess.rpy:976
translate spanish mas_chess_remenu_f696f215:

    # m 1etu "So which mode do you prefer? {w=0.3}{nw}"
    m 1etu "Entonces, ¿qué modo prefieres? {w=0.3}{nw}"

# game/chess.rpy:977
translate spanish mas_chess_remenu_278fdfc1:

    # extend 1hub "Ahaha~"
    extend 1hub "Jajaja~"

# game/chess.rpy:988
translate spanish mas_chess_remenu_1e07b804:

    # m 1eua "If we play with casual rules, we just won't count stalemates as draws.{w=0.2} {nw}"
    m 1eua "Si jugamos con reglas casuales, simplemente no contaremos el ahogado como tablas. {w=0.2}{nw}"

# game/chess.rpy:989
translate spanish mas_chess_remenu_1c1f75b5:

    # extend 3eub "Essentially, the player who is not trapped is declared the winner."
    extend 3eub "Básicamente, el jugador que no está acorralado es declarado ganador."

# game/chess.rpy:1068
translate spanish mas_chess_start_chess_0cc24a7f:

    # m 1ekc "Don't give up so easily..."
    m 1ekc "No te rindas tan fácilmente..."

# game/chess.rpy:1069
translate spanish mas_chess_start_chess_6f1757c0:

    # m 1eka "I'm sure if you keep trying, you can beat me."
    m 1eka "Estoy segura de que si sigues intentándolo, podrás vencerme."

# game/chess.rpy:1070
translate spanish mas_chess_start_chess_f15775ec:

    # m 1ekc "..."
    m 1ekc "..."

# game/chess.rpy:1071
translate spanish mas_chess_start_chess_4fc7f6cc:

    # m 1eka "I hope you don't get frustrated when you play with me."
    m 1eka "Espero que no te frustres cuando juegues conmigo."

# game/chess.rpy:1072
translate spanish mas_chess_start_chess_8d8da5bc:

    # m 3ekb "It really means a lot to me that you keep playing if you do~"
    m 3ekb "Realmente significa mucho para mí que sigas jugando conmigo~"

# game/chess.rpy:1073
translate spanish mas_chess_start_chess_ff4c9b5d:

    # m 3hua "Let's play again soon, alright?"
    m 3hua "Juguemos de nuevo pronto, ¿de acuerdo?"

# game/chess.rpy:1076
translate spanish mas_chess_start_chess_0234ec52:

    # m 1ekc "Giving up, [player]?"
    m 1ekc "¿Te rindes, [player]?"

# game/chess.rpy:1077
translate spanish mas_chess_start_chess_77e4d9e7:

    # m 1eub "Alright, but even if things aren't going too well, it's more fun to play to the end!"
    m 1eub "Está bien, pero aunque las cosas no vayan muy bien, ¡es más divertido jugar hasta el final!"

# game/chess.rpy:1078
translate spanish mas_chess_start_chess_6586871a:

    # m 3eka "In the end, I'm just happy to be spending time with you~"
    m 3eka "Al final, me hace feliz pasar tiempo contigo~"

# game/chess.rpy:1079
translate spanish mas_chess_start_chess_fe9c0937:

    # m 1eua "Anyway..."
    m 1eua "De todos modos..."

# game/chess.rpy:1083
translate spanish mas_chess_start_chess_2377db7a:

    # m 1sub "I won, yay!~"
    m 1sub "¡Sí, gané!~"

# game/chess.rpy:1096
translate spanish mas_chess_start_chess_fe9c0937_1:

    # m 1eua "Anyway..."
    m 1eua "De todos modos..."

# game/chess.rpy:1110
translate spanish mas_chess_start_chess_ed69febf:

    # m 1eua "Sure, we'll call this game a draw."
    m 1eua "Claro, diremos que esta partida es un empate."

# game/chess.rpy:1111
translate spanish mas_chess_start_chess_7fc09fe5:

    # m 3wuo "That was a pretty long game!"
    m 3wuo "¡Ha sido una partida bastante larga!"

# game/chess.rpy:1115
translate spanish mas_chess_start_chess_dacd03c3:

    # m 1eka "Aw, looks like we have a stalemate."
    m 1eka "Ay, parece que terminamos en ahogado."

# game/chess.rpy:1119
translate spanish mas_chess_start_chess_52127a00:

    # m 3hub "[line_start], you're getting closer and closer to beating me, [player]~"
    m 3hub "[line_start!t], te estás acercando cada vez más a vencerme, [player]~"

# game/chess.rpy:1122
translate spanish mas_chess_start_chess_27c4f439:

    # m 1hua "Nice work on getting this far, [player]~"
    m 1hua "Buen trabajo llegando hasta aquí, [player]~"

# game/chess.rpy:1146
translate spanish mas_chess_start_chess_8af96f98:

    # m 3hub "Congratulations [player], you won!"
    m 3hub "¡Felicidades [player], has ganado!"

# game/chess.rpy:1150
translate spanish mas_chess_start_chess_703c118d:

    # m 1wuo "You didn't undo a single move!{w=0.2} {nw}"
    m 1wuo "¡No deshiciste ni un solo movimiento! {w=0.2}{nw}"

# game/chess.rpy:1151
translate spanish mas_chess_start_chess_bfd152d9:

    # extend 3hub "That's amazing!"
    extend 3hub "¡Eso es increíble!"

# game/chess.rpy:1154
translate spanish mas_chess_start_chess_30b0216b:

    # m 1hua "You only undid once. {w=0.2}{nw}"
    m 1hua "Solo deshiciste una vez. {w=0.2}{nw}"

# game/chess.rpy:1155
translate spanish mas_chess_start_chess_4fb0b0f8:

    # extend 3hub "Great job!"
    extend 3hub "¡Buen trabajo!"

# game/chess.rpy:1158
translate spanish mas_chess_start_chess_231fdb4f:

    # m 1hua "You only undid [undo_count] times too, great job."
    m 1hua "Y solo deshiciste [undo_count] veces, buen trabajo."

# game/chess.rpy:1161
translate spanish mas_chess_start_chess_436c0307:

    # m 1eua "[undo_count] undos, not bad at all. If we keep practicing together, I'm sure we can lower that~"
    m 1eua "[undo_count] movimientos deshechos, nada mal. Si seguimos practicando junt[o_a]s, estoy segura de que podremos bajar ese número~"

# game/chess.rpy:1164
translate spanish mas_chess_start_chess_f10da59f:

    # m 1eka "You undid [undo_count] moves though.{w=0.3} {nw}"
    m 1eka "Aunque deshiciste [undo_count] movimientos. {w=0.3}{nw}"

# game/chess.rpy:1165
translate spanish mas_chess_start_chess_ca6d958e:

    # extend 3eua "But I'm sure if we keep practicing, we can get that number lower."
    extend 3eua "Pero estoy segura de que si seguimos practicando, podremos bajar ese número."

# game/chess.rpy:1167
translate spanish mas_chess_start_chess_83fdee0b:

    # m 3hua "[player_win_quip]"
    m 3hua "[player_win_quip]"

# game/chess.rpy:1170
translate spanish mas_chess_start_chess_ba77caa8:

    # m 3eub "Great job, you won!"
    m 3eub "¡Buen trabajo, has ganado!"

# game/chess.rpy:1171
translate spanish mas_chess_start_chess_0fb07fa2:

    # m 3hub "[player_win_quip]"
    m 3hub "[player_win_quip]"

# game/chess.rpy:1173
translate spanish mas_chess_start_chess_fe9c0937_2:

    # m 1eua "Anyway..."
    m 1eua "De todos modos..."

# game/chess.rpy:1188
translate spanish mas_chess_start_chess_673bb300:

    # m 1eua "Would you like to save this game?{nw}"
    m 1eua "¿Te gustaría guardar esta partida?{nw}"

# game/chess.rpy:1191
translate spanish mas_chess_start_chess_922efe72:

    # m "Would you like to save this game?{fast}" nointeract
    m "¿Te gustaría guardar esta partida?{fast}" nointeract

# game/chess.rpy:1202
translate spanish mas_chess_play_again_ask_52b51855:

    # m 1eua "Would you like to play again?{nw}"
    m 1eua "¿Te gustaría jugar de nuevo?{nw}"

# game/chess.rpy:1205
translate spanish mas_chess_play_again_ask_32d0248c:

    # m "Would you like to play again?{fast}" nointeract
    m "¿Te gustaría jugar de nuevo?{fast}" nointeract

# game/chess.rpy:1219
translate spanish mas_chess_play_again_ask_ea210635:

    # m 1eua "Alright, let's play again soon."
    m 1eua "Está bien, juguemos de nuevo pronto."

# game/chess.rpy:1230
translate spanish mas_chess_draw_lots_47608234:

    # m 2eub "Oh look, I drew black![lets_begin]"
    m 2eub "¡Oh, mira, me han tocado las negras![lets_begin!t]"

# game/chess.rpy:1233
translate spanish mas_chess_draw_lots_72a19603:

    # m 2eub "Oh look, I drew white![lets_begin]"
    m 2eub "¡Oh, mira, me han tocado las blancas![lets_begin!t]"

# game/chess.rpy:1287
translate spanish mas_chess_savegame_save_start_8375815b:

    # m 1eka "We already have a game named '[save_name].'"
    m 1eka "Ya tenemos una partida llamada '[save_name]'."

# game/chess.rpy:1289
translate spanish mas_chess_savegame_save_start_c70564be:

    # m "Should I overwrite it?{nw}"
    m "¿Quieres que la sobreescriba?{nw}"

# game/chess.rpy:1292
translate spanish mas_chess_savegame_save_start_527218b8:

    # m "Should I overwrite it?{fast}" nointeract
    m "¿Quieres que la sobreescriba?{fast}" nointeract

# game/chess.rpy:1315
translate spanish mas_chess_savegame_save_start_2c941545:

    # m 1dsc ".{w=0.5}.{w=0.5}.{nw}"
    m 1dsc ".{w=0.5}.{w=0.5}.{w=0.5}{nw}"

# game/chess.rpy:1316
translate spanish mas_chess_savegame_save_start_3cffd949:

    # m 1hua "I've saved our game in '[display_file_path]'!"
    m 1hua "¡He guardado nuestra partida en '[display_file_path]'!"

# game/chess.rpy:1322
translate spanish mas_chess_savegame_pgn_explain_27b254e6:

    # m 1esa "It's in a format called 'Portable Game Notation.'{w=0.2} {nw}"
    m 1esa "Está en un formato llamado 'Portable Game Notation'. {w=0.2}{nw}"

# game/chess.rpy:1323
translate spanish mas_chess_savegame_pgn_explain_ebc860c3:

    # extend 1eua "You can find PGN analyzers online to open it and see where you made your mistakes."
    extend 1eua "Puedes encontrar analizadores de PGN en internet para abrirlo y ver dónde cometiste errores."

# game/chess.rpy:1324
translate spanish mas_chess_savegame_pgn_explain_35985a6f:

    # m 3eub "Whether you win, lose, surrender, or draw, there's always something you could've done better, so loading these games up can really help you improve!"
    m 3eub "Ya sea que ganes, pierdas, te rindas o empates, siempre hay algo que podrías haber hecho mejor, ¡así que cargar estas partidas puede ayudarte a mejorar mucho!"

# game/chess.rpy:1327
translate spanish mas_chess_savegame_pgn_explain_e49b6220:

    # m 1lksdlb "It's possible to edit this file and change the outcome of the game...{w=0.5} {nw}"
    m 1lksdlb "Es posible editar este archivo y cambiar el resultado de la partida... {w=0.5}{nw}"

# game/chess.rpy:1328
translate spanish mas_chess_savegame_pgn_explain_0c85f20b:

    # extend 1tsu "but I'm sure you wouldn't do that."
    extend 1tsu "pero estoy segura de que tú no harías eso."

# game/chess.rpy:1330
translate spanish mas_chess_savegame_pgn_explain_e897b6e6:

    # m 1tku "Right, [player]?{nw}"
    m 1tku "¿Verdad, [player]?{nw}"

# game/chess.rpy:1333
translate spanish mas_chess_savegame_pgn_explain_a013343e:

    # m "Right, [player]?{fast}" nointeract
    m "¿Verdad, [player]?{fast}" nointeract

# game/chess.rpy:1336
translate spanish mas_chess_savegame_pgn_explain_c46167fd:

    # m 1hua "Yay~"
    m 1hua "Yey~"

# game/chess.rpy:1339
translate spanish mas_chess_savegame_pgn_explain_47e99198:

    # m 1eub "Let's continue this game soon!"
    m 1eub "¡Continuemos esta partida pronto!"

# game/chess.rpy:1343
translate spanish mas_chess_locked_no_play_092519a6:

    # m 1euc "No thanks, [player]."
    m 1euc "No gracias, [player]."

# game/chess.rpy:1344
translate spanish mas_chess_locked_no_play_3b4636eb:

    # m 1rsc "I don't really feel like playing chess right now."
    m 1rsc "No me apetece mucho jugar al ajedrez ahora mismo."

# game/chess.rpy:1351
translate spanish mas_chess_cannot_work_embarrassing_7faca44c:

    # m 1rksdla "..."
    m 1rksdla "..."

# game/chess.rpy:1352
translate spanish mas_chess_cannot_work_embarrassing_4bdc0c42:

    # m 3hksdlb "Well that's embarrassing, it seems I can't actually get chess to work on your system..."
    m 3hksdlb "Vaya, esto es vergonzoso, parece que no puedo hacer que el ajedrez funcione en tu sistema..."

# game/chess.rpy:1353
translate spanish mas_chess_cannot_work_embarrassing_a605df73:

    # m 1ekc "Sorry about that, [player]."
    m 1ekc "Lo siento por eso, [player]."

# game/chess.rpy:1354
translate spanish mas_chess_cannot_work_embarrassing_48c7dc74:

    # m 1eka "Maybe we can do something else instead?"
    m 1eka "¿Qué tal si hacemos otra cosa?"

# game/chess.rpy:1358
translate spanish mas_chess_dlg_game_monika_wins_often_455cb71e:

    # m 1eka "Sorry you didn't win this time, [player]..."
    m 1eka "Siento que no hayas podido ganar esta vez, [player]..."

# game/chess.rpy:1359
translate spanish mas_chess_dlg_game_monika_wins_often_f8896e52:

    # m 1ekc "I hope you'll at least keep trying though."
    m 1ekc "Espero que al menos sigas intentándolo."

# game/chess.rpy:1360
translate spanish mas_chess_dlg_game_monika_wins_often_5085d66b:

    # m 1eua "Let's play again soon, okay?"
    m 1eua "Juguemos de nuevo pronto, ¿de acuerdo?"

# game/chess.rpy:1363
translate spanish mas_chess_dlg_game_monika_wins_often_b45791e5:

    # m 1hua "You'll beat me someday~"
    m 1hua "Algún día me ganarás~"

# game/chess.rpy:1367
translate spanish mas_chess_dlg_game_monika_wins_sometimes_c3dc3c16:

    # m 1hub "That was really fun, [player]!"
    m 1hub "¡Eso fue muy divertido, [player]!"

# game/chess.rpy:1368
translate spanish mas_chess_dlg_game_monika_wins_sometimes_92acc1dc:

    # m 3eka "No matter the outcome, I always enjoy playing chess with you~"
    m 3eka "No importa el resultado, siempre disfruto jugando al ajedrez contigo~"

# game/chess.rpy:1369
translate spanish mas_chess_dlg_game_monika_wins_sometimes_b275f0cd:

    # m 3hua "I bet if you keep practicing, you'll be even better than me someday!"
    m 3hua "¡Apuesto a que si sigues practicando, algún día serás incluso mejor que yo!"

# game/chess.rpy:1373
translate spanish mas_chess_dlg_game_monika_wins_sometimes_aaa14f22:

    # m 3eua "Until then though, I'll try and go a little easier on you."
    m 3eua "Pero hasta entonces, intentaré ponértelo un poco más fácil."

# game/chess.rpy:1424
translate spanish mas_chess_save_multi_dlg_c7b7e8f2:

    # m 1eua "So I've been thinking, [player]..."
    m 1eua "He estado pensando, [player]..."

# game/chess.rpy:1425
translate spanish mas_chess_save_multi_dlg_06987ede:

    # m 1euc "Most people who leave in the middle of a chess game don't come back to start a new one."
    m 1euc "La mayoría de personas que dejan una partida de ajedrez pendiente no suelen volver para empezar otra."

# game/chess.rpy:1426
translate spanish mas_chess_save_multi_dlg_6496b704:

    # m 3eud "...So it makes no sense for me to keep track of more than one unfinished game between us."
    m 3eud "... Así que no tiene sentido que guarde más de una partida pendiente entre nosotr[o_a]s."

# game/chess.rpy:1427
translate spanish mas_chess_save_multi_dlg_28af9245:

    # m 1rka "And since we have [game_count] games in progress..."
    m 1rka "Y como tenemos [game_count] partidas en curso..."

# game/chess.rpy:1428
translate spanish mas_chess_save_multi_dlg_ccb20cfd:

    # m 3euc "I have to ask you to pick only one to keep.{w=0.2} Sorry, [player]."
    m 3euc "Tengo que pedirte que elijas solo una para conservar. {w=0.2}Lo siento, [player]."

# game/chess.rpy:1439
translate spanish mas_chess_save_multi_dlg_6be9d23e:

    # m 2dsc "I see."
    m 2dsc "Entiendo."

# game/chess.rpy:1440
translate spanish mas_chess_save_multi_dlg_a29fd4bb:

    # m 2eua "In that case, please take your time."
    m 2eua "En ese caso, por favor tómate tu tiempo."

# game/chess.rpy:1441
translate spanish mas_chess_save_multi_dlg_c739d190:

    # m 1eua "We'll play chess again once you've made your decision."
    m 1eua "Volveremos a jugar al ajedrez una vez que hayas tomado tu decisión."

# game/chess.rpy:1446
translate spanish mas_chess_save_multi_dlg_0f026494:

    # m 1eua "Alright."
    m 1eua "Está bien."

# game/chess.rpy:1473
translate spanish mas_chess_dlg_quicksave_lost_53ee25dd:

    # m 2lksdlb "Uh, [player]...{w=0.5} I think I messed up in saving our last game, and now I can't open it anymore."
    m 2lksdlb "Eh, [player]... {w=0.5}creo que me equivoqué al guardar nuestra última partida, y ahora ya no puedo abrirla."

# game/chess.rpy:1476
translate spanish mas_chess_dlg_quicksave_lost_db12ef2e:

    # m 1lksdld "I'm really, really sorry, [player]..."
    m 1lksdld "Lo siento mucho, de verdad, [player]..."

# game/chess.rpy:1479
translate spanish mas_chess_dlg_quicksave_lost_d22ee58b:

    # m 1eka "But don't worry, I'll make it up to you...{w=0.3}{nw}"
    m 1eka "Pero no te preocupes, te lo compensaré... {w=0.3}{nw}"

# game/chess.rpy:1480
translate spanish mas_chess_dlg_quicksave_lost_d8c79967:

    # extend 3hua "by starting a new game!"
    extend 3hua "¡empezando una partida nueva!"

# game/chess.rpy:1481
translate spanish mas_chess_dlg_quicksave_lost_0fb41a85:

    # m 3hub "Ahaha~"
    m 3hub "Jajaja~"

# game/chess.rpy:1484
translate spanish mas_chess_dlg_quicksave_lost_297eb97f:

    # m 1lksdlc "I'm so clumsy, [player]...{w=0.3} I'm sorry."
    m 1lksdlc "Soy tan torpe, [player]... {w=0.3}lo siento."

# game/chess.rpy:1485
translate spanish mas_chess_dlg_quicksave_lost_cefb8c08:

    # m 3eksdla "Let's start a new game instead."
    m 3eksdla "En su lugar, empecemos una partida nueva."

# game/chess.rpy:1488
translate spanish mas_chess_dlg_quicksave_lost_39072f4d:

    # m 2esc "This has happened [qs_gone_count] times now..."
    m 2esc "Ya van [qs_gone_count] veces que pasa esto..."

# game/chess.rpy:1489
translate spanish mas_chess_dlg_quicksave_lost_cf8a6a73:

    # m 2tsc "I wonder if this is a side effect of {i}someone{/i} trying to edit the saves.{w=1}.{w=1}."
    m 2tsc "Me pregunto si esto es consecuencia de que {i}alguien{/i} intentó editar los archivos de guardado.{w=1}.{w=1}.{w=1}"

# game/chess.rpy:1490
translate spanish mas_chess_dlg_quicksave_lost_0eae88ec:

    # m 7rsc "Anyway..."
    m 7rsc "En fin..."

# game/chess.rpy:1491
translate spanish mas_chess_dlg_quicksave_lost_61e666c3:

    # m 1esc "Let's start a new game."
    m 1esc "Empecemos una partida nueva."

# game/chess.rpy:1494
translate spanish mas_chess_dlg_quicksave_lost_a7509a9c:

    # m 1lksdlc "I'm sorry..."
    m 1lksdlc "Lo siento..."

# game/chess.rpy:1495
translate spanish mas_chess_dlg_quicksave_lost_41af46d5:

    # m 3eka "Let's start a new game instead."
    m 3eka "En su lugar, empecemos una partida nueva."

# game/chess.rpy:1503
translate spanish mas_chess_dlg_quickfile_lost_ea169cc7:

    # m 2lksdla "Well this is embarrassing..."
    m 2lksdla "Vaya, esto es vergonzoso..."

# game/chess.rpy:1504
translate spanish mas_chess_dlg_quickfile_lost_8e0a9aac:

    # m 2ekc "I could have sworn that we had an unfinished game, but I can't find the save file."
    m 2ekc "Podría haber jurado que teníamos una partida pendiente, pero no encuentro el archivo de guardado."

# game/chess.rpy:1506
translate spanish mas_chess_dlg_quickfile_lost_1c3a5d15:

    # m 2tkc "Did you mess with the saves, [player]?{nw}"
    m 2tkc "¿Estuviste manipulando los archivos de guardado, [player]?{nw}"

# game/chess.rpy:1509
translate spanish mas_chess_dlg_quickfile_lost_f27f6469:

    # m "Did you mess with the saves, [player]?{fast}" nointeract
    m "¿Estuviste manipulando los archivos de guardado, [player]?{fast}" nointeract

# game/chess.rpy:1526
translate spanish mas_chess_dlg_quickfile_lost_deleted_e87f814d:

    # m 1eka "Thanks for being honest with me, [player]."
    m 1eka "Gracias por ser sincero conmigo, [player]."

# game/chess.rpy:1528
translate spanish mas_chess_dlg_quickfile_lost_deleted_7fc2ae0b:

    # m 3ekd "Did you not want to continue that game?{nw}"
    m 3ekd "¿No querías continuar esa partida?{nw}"

# game/chess.rpy:1531
translate spanish mas_chess_dlg_quickfile_lost_deleted_ae2adb13:

    # m "Did you not want to continue that game?{fast}" nointeract
    m "¿No querías continuar esa partida?{fast}" nointeract

# game/chess.rpy:1534
translate spanish mas_chess_dlg_quickfile_lost_deleted_eb42e33a:

    # m 1eka "I understand, [player]."
    m 1eka "Lo entiendo, [player]."

# game/chess.rpy:1535
translate spanish mas_chess_dlg_quickfile_lost_deleted_33c3dc8e:

    # m 1hua "Let's start a new game~"
    m 1hua "Empecemos una partida nueva~"

# game/chess.rpy:1538
translate spanish mas_chess_dlg_quickfile_lost_deleted_ad363a17:

    # m 1etc "Oh?"
    m 1etc "¿Oh?"

# game/chess.rpy:1539
translate spanish mas_chess_dlg_quickfile_lost_deleted_0f2b82a7:

    # m 1rsc "I guess you just deleted it by mistake then."
    m 1rsc "Supongo que la borraste por error entonces."

# game/chess.rpy:1540
translate spanish mas_chess_dlg_quickfile_lost_deleted_19a3e417:

    # m 1eua "Let's just start a new game."
    m 1eua "Simplemente empecemos una partida nueva."

# game/chess.rpy:1550
translate spanish mas_chess_dlg_quickfile_lost_ofcoursenot_5b0ed382:

    # m 2esc "..."
    m 2esc "..."

# game/chess.rpy:1551
translate spanish mas_chess_dlg_quickfile_lost_ofcoursenot_575df7d4:

    # m "[player],{w=0.2} did you..."
    m "[player], {w=0.2}acaso tú..."

# game/chess.rpy:1552
translate spanish mas_chess_dlg_quickfile_lost_ofcoursenot_87a07ff4:

    # m 2dsc "Nevermind."
    m 2dsc "Olvídalo."

# game/chess.rpy:1553
translate spanish mas_chess_dlg_quickfile_lost_ofcoursenot_16a2e1d9:

    # m 1esc "Let's play a new game."
    m 1esc "Juguemos una partida nueva."

# game/chess.rpy:1557
translate spanish mas_chess_dlg_quickfile_lost_ofcoursenot_5b0ed382_1:

    # m 2esc "..."
    m 2esc "..."

# game/chess.rpy:1558
translate spanish mas_chess_dlg_quickfile_lost_ofcoursenot_8d90c5b3:

    # m "[player],{w=0.2} this is happening way too much."
    m "[player], {w=0.2}esto está pasando demasiadas veces."

# game/chess.rpy:1559
translate spanish mas_chess_dlg_quickfile_lost_ofcoursenot_89fbc396:

    # m 2dsc "I really don't believe you this time."
    m 2dsc "Esta vez realmente no te creo."

# game/chess.rpy:1561
translate spanish mas_chess_dlg_quickfile_lost_ofcoursenot_a13f8ce2:

    # m 2esc "I hope you're not messing with me."
    m 2esc "Espero que no me estés mintiendo."

# game/chess.rpy:1562
translate spanish mas_chess_dlg_quickfile_lost_ofcoursenot_e612037c:

    # m "..."
    m "..."

# game/chess.rpy:1563
translate spanish mas_chess_dlg_quickfile_lost_ofcoursenot_f4e089fa:

    # m 1esc "Whatever.{w=0.5} Let's just play a new game."
    m 1esc "Como sea. {w=0.5}Solo juguemos una nueva partida."

# game/chess.rpy:1575
translate spanish mas_chess_dlg_quickfile_lost_ofcoursenot_01a93df6:

    # m 2dfc "..."
    m 2dfc "..."

# game/chess.rpy:1576
translate spanish mas_chess_dlg_quickfile_lost_ofcoursenot_68ab2144:

    # m 2efc "[player],{w=0.3} I don't believe you."
    m 2efc "[player], {w=0.3}no te creo."

# game/chess.rpy:1577
translate spanish mas_chess_dlg_quickfile_lost_ofcoursenot_746a9094:

    # m 2efd "If you're just going to throw away our chess games like that..."
    m 2efd "Si vas a desechar nuestras partidas de ajedrez de esa forma..."

# game/chess.rpy:1578
translate spanish mas_chess_dlg_quickfile_lost_ofcoursenot_5551b89f:

    # m 6wfw "Then I don't want to play chess with you anymore!"
    m 6wfw "¡Entonces ya no quiero jugar más al ajedrez contigo!"

# game/chess.rpy:1582
translate spanish mas_chess_dlg_quickfile_lost_ofcoursenot_6be49146:

    # m 1lksdlb "Ah, yeah. You wouldn't do that to me."
    m 1lksdlb "Ah, sí. Tú no me harías eso."

# game/chess.rpy:1583
translate spanish mas_chess_dlg_quickfile_lost_ofcoursenot_1eaeb43d:

    # m "I must have misplaced the save file."
    m "Debo haber extraviado el archivo de guardado."

# game/chess.rpy:1584
translate spanish mas_chess_dlg_quickfile_lost_ofcoursenot_235ad172:

    # m 1lksdlc "Sorry, [player]."
    m 1lksdlc "Lo siento, [player]."

# game/chess.rpy:1585
translate spanish mas_chess_dlg_quickfile_lost_ofcoursenot_fac6bd8d:

    # m 1eka "I'll make it up to you...{w=0.3}{nw}"
    m 1eka "Te lo compensaré... {w=0.3}{nw}"

# game/chess.rpy:1586
translate spanish mas_chess_dlg_quickfile_lost_ofcoursenot_6abb7f43:

    # extend 1eub "by starting a new game!"
    extend 1eub "¡empezando una partida nueva!"

# game/chess.rpy:1598
translate spanish mas_chess_dlg_quickfile_lost_maybe_1cfb3648:

    # m 2ekd "[player]!{w=0.2} I should have known you were just messing with me!"
    m 2ekd "¡[player]! {w=0.2}¡Debí saber que solo te estabas burlando de mí!"

# game/chess.rpy:1602
translate spanish mas_chess_dlg_quickfile_lost_maybe_8d0003be:

    # m 2ekd "[player]!{w=0.2} Stop messing with me!"
    m 2ekd "¡[player]! {w=0.2}¡Deja de burlarte de mí!"

# game/chess.rpy:1608
translate spanish mas_chess_dlg_quickfile_lost_maybe_101d3f95:

    # m 2ekd "[player]! That's--"
    m 2ekd "¡[player]! Eso es..."

# game/chess.rpy:1609
translate spanish mas_chess_dlg_quickfile_lost_maybe_c61f257d:

    # m 2dkc "..."
    m 2dkc "..."

# game/chess.rpy:1610
translate spanish mas_chess_dlg_quickfile_lost_maybe_8aca988f:

    # m 1esa "...not a problem at all."
    m 1esa "... No hay problema."

# game/chess.rpy:1611
translate spanish mas_chess_dlg_quickfile_lost_maybe_2b30b07a:

    # m "I knew you were going to do this again..."
    m "Sabía que ibas a hacer esto otra vez..."

# game/chess.rpy:1612
translate spanish mas_chess_dlg_quickfile_lost_maybe_3423dac4:

    # m 1hub "...so I kept a backup of our save!"
    m 1hub "... ¡Así que guardé una copia de seguridad de nuestra partida!"

# game/chess.rpy:1613
translate spanish mas_chess_dlg_quickfile_lost_maybe_2a2f55d8:

    # m 1kua "You can't trick me anymore, [player]."
    m 1kua "Ya no puedes engañarme, [player]."

# game/chess.rpy:1614
translate spanish mas_chess_dlg_quickfile_lost_maybe_93e4cfa7:

    # m "Now let's continue our game."
    m "Ahora continuemos nuestra partida."

# game/chess.rpy:1625
translate spanish mas_chess_quickfile_lost_filechecker_88cfc3f2:

    # m 1eka "Can you put the save back so we can play?"
    m 1eka "¿Puedes volver a poner el archivo para que podamos jugar?"

# game/chess.rpy:1658
translate spanish mas_chess_dlg_quickfile_lost_maybe_filechecker_file_found_57741063:

    # m 1hua "Yay!{w=0.2} Thanks for putting it back, [player]."
    m 1hua "¡Yey! {w=0.2}Gracias por volver a ponerlo, [player]."

# game/chess.rpy:1659
translate spanish mas_chess_dlg_quickfile_lost_maybe_filechecker_file_found_0fb32012:

    # m "Now we can continue our game."
    m "Ahora podemos continuar nuestra partida."

# game/chess.rpy:1664
translate spanish mas_chess_dlg_quickfile_lost_maybe_filechecker_no_file_6e92af18:

    # m 1ekd "[player]..."
    m 1ekd "[player]..."

# game/chess.rpy:1665
translate spanish mas_chess_dlg_quickfile_lost_maybe_filechecker_no_file_ac9ba018:

    # m 1eka "That's okay. Let's just play a new game."
    m 1eka "Está bien. Juguemos una partida nueva."

# game/chess.rpy:1670
translate spanish mas_chess_dlg_quickfile_lost_maybe_save_found_9da041f8:

    # m 2eua "Oh!"
    m 2eua "¡Oh!"

# game/chess.rpy:1671
translate spanish mas_chess_dlg_quickfile_lost_maybe_save_found_b226c195:

    # m 1hua "There's the save.{w=0.2} Thanks for putting it back, [player]."
    m 1hua "Ahí está la partida. {w=0.2}Gracias por volver a ponerla, [player]."

# game/chess.rpy:1672
translate spanish mas_chess_dlg_quickfile_lost_maybe_save_found_231a42ed:

    # m 1eua "Now we can continue our game."
    m 1eua "Ahora podemos continuar nuestra partida."

# game/chess.rpy:1682
translate spanish mas_chess_dlg_quickfile_lost_accident_7fd8f6bf:

    # m 1eka "Again? Don't be so clumsy, [player]."
    m 1eka "¿Otra vez? No seas tan torpe, [player]."

# game/chess.rpy:1683
translate spanish mas_chess_dlg_quickfile_lost_accident_0352edc8:

    # m 1hua "But that's okay."
    m 1hua "Pero todo está bien."

# game/chess.rpy:1684
translate spanish mas_chess_dlg_quickfile_lost_accident_986783fc:

    # m 1eua "We'll just play a new game instead."
    m 1eua "Mejor juguemos una partida nueva."

# game/chess.rpy:1688
translate spanish mas_chess_dlg_quickfile_lost_accident_30c8f345:

    # m 1eka "I had a feeling this would happen again."
    m 1eka "Tenía el presentimiento de que esto volvería a pasar."

# game/chess.rpy:1689
translate spanish mas_chess_dlg_quickfile_lost_accident_3e764a5c:

    # m 3tub "So I kept a backup of our save!"
    m 3tub "¡Así que guardé una copia de seguridad de nuestra partida!"

# game/chess.rpy:1690
translate spanish mas_chess_dlg_quickfile_lost_accident_2b15dcb3:

    # m 1hua "Now we can continue our game~"
    m 1hua "Ahora podemos continuar nuestra partida~"

# game/chess.rpy:1694
translate spanish mas_chess_dlg_quickfile_lost_accident_3278aa0f:

    # m 1ekc "[player]...{w=0.3} {nw}"
    m 1ekc "[player]... {w=0.3}{nw}"

# game/chess.rpy:1695
translate spanish mas_chess_dlg_quickfile_lost_accident_6cc88427:

    # extend 1eka "That's okay.{w=0.3} Accidents happen."
    extend 1eka "está bien. {w=0.3}Los accidentes pasan."

# game/chess.rpy:1696
translate spanish mas_chess_dlg_quickfile_lost_accident_2897c47d:

    # m 1eua "Let's play a new game instead."
    m 1eua "Mejor juguemos una partida nueva."

# game/chess.rpy:1702
translate spanish mas_chess_dlg_quickfile_edited_6f96a035:

    # m 2lksdlc "[player]..."
    m 2lksdlc "[player]..."

# game/chess.rpy:1704
translate spanish mas_chess_dlg_quickfile_edited_76684384:

    # m 2ekc "Did you edit the save file?{nw}"
    m 2ekc "¿Editaste el archivo de guardado?{nw}"

# game/chess.rpy:1707
translate spanish mas_chess_dlg_quickfile_edited_d8d1b624:

    # m "Did you edit the save file?{fast}" nointeract
    m "¿Editaste el archivo de guardado?{fast}" nointeract

# game/chess.rpy:1723
translate spanish mas_chess_dlg_quickfile_edited_yes_85c15399:

    # m 1dsc "I'm disappointed in you."
    m 1dsc "Estoy decepcionada de ti."

# game/chess.rpy:1724
translate spanish mas_chess_dlg_quickfile_edited_yes_84524dae:

    # m 1eka "But I'm glad that you were honest with me."
    m 1eka "Pero me alegra que hayas sido honesto conmigo."

# game/chess.rpy:1733
translate spanish mas_chess_dlg_quickfile_edited_yes_19b04a78:

    # m 1hua "Apology accepted!"
    m 1hua "¡Disculpas aceptadas!"

# game/chess.rpy:1734
translate spanish mas_chess_dlg_quickfile_edited_yes_eb17b3ec:

    # m 1eua "Luckily, I still remember a little bit of the last game, so we can continue it from there."
    m 1eua "Por suerte, todavía recuerdo un poco de la última partida, así que podemos continuarla desde ahí."

# game/chess.rpy:1742
translate spanish mas_chess_dlg_quickfile_edited_yes_game_ruined_59588614:

    # m 1lfc "Since that game's been ruined, let's just play a new game."
    m 1lfc "Como esa partida se ha arruinado, juguemos una partida nueva."

# game/chess.rpy:1749
translate spanish mas_chess_dlg_quickfile_edited_yes_game_ruined_1698f73d:

    # m 2dfc "I am incredibly disappointed in you..."
    m 2dfc "Estoy increíblemente decepcionada de ti..."

# game/chess.rpy:1750
translate spanish mas_chess_dlg_quickfile_edited_yes_game_ruined_aa12a669:

    # m 2rfc "Let's play chess some other time.{w=0.2} I don't feel like playing right now."
    m 2rfc "Jugaremos al ajedrez en otro momento. {w=0.2}No me apetece jugar ahora."

# game/chess.rpy:1757
translate spanish mas_chess_dlg_quickfile_edited_yes_game_ruined_33d9446e:

    # m 2dsc "I'm not surprised..."
    m 2dsc "No me sorprende..."

# game/chess.rpy:1758
translate spanish mas_chess_dlg_quickfile_edited_yes_game_ruined_7d5b4099:

    # m 2esc "But I am prepared."
    m 2esc "Pero estoy preparada."

# game/chess.rpy:1759
translate spanish mas_chess_dlg_quickfile_edited_yes_game_ruined_f2e38c65:

    # m 7esc "I kept a backup of our game just in case you did this again."
    m 7esc "Guardé una copia de seguridad de nuestra partida por si volvías a hacer esto."

# game/chess.rpy:1760
translate spanish mas_chess_dlg_quickfile_edited_yes_game_ruined_349cff1b:

    # m 1esa "Now let's finish this game."
    m 1esa "Ahora terminemos esta partida."

# game/chess.rpy:1775
translate spanish mas_chess_dlg_quickfile_edited_no_aee3d749:

    # m 1dsc "Hmm..."
    m 1dsc "Hmm..."

# game/chess.rpy:1776
translate spanish mas_chess_dlg_quickfile_edited_no_fcb94333:

    # m 1etc "The save file looks different from how I last remembered it,{w=0.2} {nw}"
    m 1etc "El archivo de guardado parece diferente a como lo recordaba la última vez, {w=0.2}{nw}"

# game/chess.rpy:1777
translate spanish mas_chess_dlg_quickfile_edited_no_8b2d1293:

    # extend 1rksdlc "{nw}but maybe that's just my memory failing me..."
    extend 1rksdlc "{nw}pero quizás solo sea que me falla la memoria..."

# game/chess.rpy:1778
translate spanish mas_chess_dlg_quickfile_edited_no_9f55c7c9:

    # m 1eua "Let's continue this game."
    m 1eua "Continuemos esta partida."

# game/chess.rpy:1784
translate spanish mas_chess_dlg_quickfile_edited_no_9f7abd79:

    # m 1ekc "I see."
    m 1ekc "Entiendo."

# game/chess.rpy:1785
translate spanish mas_chess_dlg_quickfile_edited_no_e612037c:

    # m "..."
    m "..."

# game/chess.rpy:1786
translate spanish mas_chess_dlg_quickfile_edited_no_402d6f88:

    # m "Let's just continue this game."
    m "Simplemente continuemos esta partida."

# game/chess.rpy:1791
translate spanish mas_chess_dlg_quickfile_edited_no_4a7ea962:

    # m 2dfc "[player]..."
    m 2dfc "[player]..."

# game/chess.rpy:1792
translate spanish mas_chess_dlg_quickfile_edited_no_f1020305:

    # m 2dftdc "I kept a backup of our game.{w=0.5} I know you edited the save file."
    m 2dftdc "Guardé una copia de seguridad de nuestra partida. {w=0.5}Sé que editaste el archivo de guardado."

# game/chess.rpy:1793
translate spanish mas_chess_dlg_quickfile_edited_no_8ab5b1d9:

    # m 6dktuc "I just-"
    m 6dktuc "Yo simplemente-"

# game/chess.rpy:1795
translate spanish mas_chess_dlg_quickfile_edited_no_6948123d:

    # m 6ektud "I just{fast} can't believe you would cheat and {i}lie{/i} to me..."
    m 6ektud "Simplemente{fast} no puedo creer que me engañaras y me {i}mintieras{/i}..."

# game/chess.rpy:1796
translate spanish mas_chess_dlg_quickfile_edited_no_d6e1d522:

    # m 6dktuc "..."
    m 6dktuc "..."

# game/chess.rpy:1813
translate spanish mas_chess_dlg_quickfile_edited_no_c3c4b6f6:

    # m 6ektpc "I forgive you, [player], but please don't do this to me again."
    m 6ektpc "Te perdono, [player], pero por favor no me vuelvas a hacer esto."

# game/chess.rpy:1814
translate spanish mas_chess_dlg_quickfile_edited_no_c55548ad:

    # m 2dktdc "..."
    m 2dktdc "..."

# game/chess.rpy:1828
translate spanish mas_chess_dlg_quickfile_edited_no_quicksave_4a7ea962:

    # m 2dfc "[player]..."
    m 2dfc "[player]..."

# game/chess.rpy:1829
translate spanish mas_chess_dlg_quickfile_edited_no_quicksave_1c17b6a4:

    # m 2tfc "I see you've edited my backup saves."
    m 2tfc "Veo que has editado mis copias de seguridad."

# game/chess.rpy:1830
translate spanish mas_chess_dlg_quickfile_edited_no_quicksave_0f9ece92:

    # m 2lfd "If you want to be like that right now, then we'll play chess some other time."
    m 2lfd "Si vas a estar con esa actitud, jugaremos al ajedrez en otro momento."

# game/chess.rpy:1840
translate spanish mas_chess_dlg_pre_go_ham_926262ec:

    # m 6ektsc "I can't trust you anymore."
    m 6ektsc "Ya no puedo confiar en ti."

# game/chess.rpy:1841
translate spanish mas_chess_dlg_pre_go_ham_5fc5ab88:

    # m 6dktsd "Goodbye, [player].{nw}"
    m 6dktsd "Adiós, [player].{nw}"

translate spanish game_chess_f85b6700:

    # m "Let's continue our unfinished game."
    m 1eub "Continuemos nuestra partida pendiente."

translate spanish strings:

    # game/chess.rpy:47
    old "{0} | {1} | Turn: {2} | You: {3}"
    new "{0} | {1} | Turno: {2} | Tú: {3}"

    # game/chess.rpy:59
    old "I can't make this decision right now..."
    new "No puedo tomar esta decisión ahora mismo..."

    # game/chess.rpy:171
    old "White"
    new "Blancas"

    # game/chess.rpy:724
    old "Practice"
    new "Practicar"

    # game/chess.rpy:874
    old "Normal Chess"
    new "Ajedrez Normal"

    # game/chess.rpy:875
    old "Randomized Chess"
    new "Ajedrez Aleatorio"

    # game/chess.rpy:876
    old "Chess 960"
    new "Ajedrez 960"

    # game/chess.rpy:878
    old "Can you explain these game-modes?"
    new "¿Puedes explicarme estos modos de juego?"

    # game/chess.rpy:881
    old "Ruleset"
    new "Reglas"

    # game/chess.rpy:882
    old "Practice or Play"
    new "Practicar o jugar"

    # game/chess.rpy:883
    old "Color"
    new "Color"

    # game/chess.rpy:884
    old "Let's play!"
    new "¡Juguemos!"

    # game/chess.rpy:890
    old "Casual Rules"
    new "Reglas informales"

    # game/chess.rpy:891
    old "Traditional Rules"
    new "Reglas tradicionales"

    # game/chess.rpy:893
    old "What's the difference?"
    new "¿Cuál es la diferencia?"

    # game/chess.rpy:896
    old "Gamemode"
    new "Modo de juego"

    # game/chess.rpy:920
    old "Let's draw lots!"
    new "¡Dejémoslo a la suerte!"

    # game/chess.rpy:1070
    old "..."
    new "..."

    # game/chess.rpy:1112
    old "Great job though"
    new "Buen trabajo"

    # game/chess.rpy:1116
    old "But on the bright side"
    new "Pero el lado bueno es que"

    # game/chess.rpy:1130
    old "I'm so proud of you, [player]!"
    new "¡Estoy tan orgullosa de ti, [player]!"

    # game/chess.rpy:1131
    old "I'm proud of you, [player]!~"
    new "¡Estoy orgullosa de ti, [player]!~"

    # game/chess.rpy:1132
    old "Well played, [player]!"
    new "¡Bien jugado, [player]!"

    # game/chess.rpy:1133
    old "It makes me really happy to see you win~"
    new "Me hace muy feliz verte ganar~"

    # game/chess.rpy:1134
    old "I'm happy to see you win!"
    new "¡Me alegra verte ganar!"

    # game/chess.rpy:1135
    old "No matter the outcome, I'll always enjoy playing with you."
    new "No importa el resultado, siempre disfrutaré jugando contigo."

    # game/chess.rpy:1214
    old "Yes, but with different rules."
    new "Sí, pero con reglas diferentes."

    # game/chess.rpy:1226
    old "{w=0.2} Let's begin."
    new "{w=0.2} Comencemos."

    # game/chess.rpy:1261
    old "Enter a name for this game:"
    new "Introduce un nombre para esta partida:"

    # game/chess.rpy:1335
    old "Of course not."
    new "Claro que no."

    # game/chess.rpy:1420
    old "You still need to pick a game to keep."
    new "Todavía tienes que elegir una partida para conservar."

    # game/chess.rpy:1429
    old "Pick a game you'd like to keep."
    new "Elige la partida que quieras conservar."

    # game/chess.rpy:1511
    old "I deleted the save."
    new "He borrado la partida."

    # game/chess.rpy:1514
    old "It was an accident!"
    new "¡Fue un accidente!"

    # game/chess.rpy:1517
    old "Maybe..."
    new "Tal vez..."

    # game/chess.rpy:1520
    old "Of course not!"
    new "¡Claro que no!"

    # game/chess.rpy:1653
    old "I deleted the save..."
    new "He borrado la partida..."

    # game/chess.rpy:1729
    old "I'm sorry."
    new "Lo siento."

    # game/chess.rpy:1917
    old "Select piece to promote to"
    new "Selecciona la pieza a la que promocionar"

    # game/chess.rpy:3248
    old "Surrender"
    new "Rendirse"

    # game/chess.rpy:3259
    old "Call Draw"
    new "Tablas"

    # game/chess.rpy:3285
    old "Undo"
    new "Deshacer"

    # game/chess.rpy:3303
    old "It's your turn, [player]."
    new "Es tu turno, [player]."

    # game/chess.rpy:3304
    old "Your move, [player]~"
    new "Te toca, [player]~"

    # game/chess.rpy:3305
    old "What will you do, I wonder..."
    new "Me pregunto qué harás..."

    # game/chess.rpy:3306
    old "Alright, your turn, [player]~"
    new "De acuerdo, es tu turno [player]~"

    # game/chess.rpy:3307
    old "You got this, [player]!"
    new "¡Tú puedes, [player]!"

    # game/chess.rpy:3310
    old "[mas_quipExp('3tfb')]Check!"
    new "[mas_quipExp('3tfb')]¡Jaque!"

    # game/chess.rpy:3311
    old "[mas_quipExp('3huu')]I've got you now, [player]!"
    new "[mas_quipExp('3huu')]¡Te tengo ahora, [player]!"

    # game/chess.rpy:3312
    old "[mas_quipExp('3hub')]Looks like you're in check!"
    new "[mas_quipExp('3hub')]¡Parece que estás en jaque!"

    # game/chess.rpy:3317
    old "Alright, let's see..."
    new "De acuerdo, veamos..."

    # game/chess.rpy:3318
    old "Okay, my turn..."
    new "Okey, mi turno..."

    # game/chess.rpy:3319
    old "Let's see what I can do."
    new "Veamos qué puedo hacer."

    # game/chess.rpy:3320
    old "I think I'll try this..."
    new "Creo que intentaré esto..."

    # game/chess.rpy:3321
    old "Okay, I'll move this here then."
    new "Okey, entonces moveré esto aquí."

    # game/chess.rpy:3324
    old "[mas_quipExp('1eusdlc')]Uh oh..."
    new "[mas_quipExp('1eusdlc')]Oh..."

    # game/chess.rpy:3325
    old "[mas_quipExp('1rksdlc')]Hmm...{w=0.2}I need to get out of this..."
    new "[mas_quipExp('1rksdlc')]Hmm... {w=0.2}necesito salir de esto..."

    # game/chess.rpy:3326
    old "[mas_quipExp('1etc')]What's the right move here..."
    new "[mas_quipExp('1etc')]¿Cuál sería la mejor jugada?..."

    # game/chess.rpy:3274
    old "Done"
    new "Listo"

    # game/chess.rpy:3410
    old "Hmm, that's odd. It seems some permissions were changed and I can't get chess running on your system."
    new "Hmm, es extraño. Parece que algunos permisos fueron cambiados y no puedo hacer funcionar el ajedrez en tu sistema."

    # game/chess.rpy:3412
    old "Hold on a second, [player]. I'm going to try something quickly.{w=0.3}.{w=0.3}.{w=0.3}{nw}"
    new "Dame un segundo, [player]. Voy a intentar algo rápidamente.{w=0.3}.{w=0.3}.{w=0.3}{nw}"

    # game/chess.rpy:3444
    old "Yay! We should be able to play now~"
    new "¡Yey! Deberíamos poder jugar ahora~"

    # game/chess.rpy:3511
    old "You'd like to continue later?"
    new "¿Te gustaría continuar más tarde?"

    # game/chess.rpy:3526
    old "Are you sure you want to give up?"
    new "¿Estás seguro de que quieres rendirte?"
